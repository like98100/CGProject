...
bool ParticleSystemClass::Initialize(ID3D11Device* device, const WCHAR* textureFilename, int index)
{
	...
	// 파티클 시스템을 초기화합니다.
	if(!InitializeParticleSystem(index))
	{
		return false;
	}
...
bool ParticleSystemClass::Frame(int playerY, float frameTime, ID3D11DeviceContext* deviceContext)
{
	// 오래된 파티클을 해제합니다.
	KillParticles(0);
...
bool ParticleSystemClass::InitializeParticleSystem(int index)
{
...
void ParticleSystemClass::EmitParticles(float frameTime)
{
	...
	if(m_accumulatedTime > (10000.0f / m_particlesPerSecond))
	{
		m_accumulatedTime = 0.0f;
		emitParticle = true;
	}
	...
	if((emitParticle == true) && (m_currentParticleCount < (m_maxParticles - 1)))
	{
		...
		float velocity = (m_particleVelocity + (((float)rand()-(float)rand())/RAND_MAX) * m_particleVelocityVariation) * (-1);
...
void ParticleSystemClass::UpdateParticles(float frameTime)
{
	...
	for(int i=0; i<m_currentParticleCount; i++)
	{
		...
		//m_particleList[i].positionX = m_particleList[i].positionX - (m_particleList[i].velocity * frameTime * 0.002f);
		m_particleList[i].positionZ = m_particleList[i].positionZ - (m_particleList[i].velocity * frameTime * 0.002f);
	}
}


void ParticleSystemClass::KillParticles(int playerY)
{
	// 특정 높이 범위를 벗어난 모든 파티클을 제거합니다.
	for(int i=0; i<m_maxParticles; i++)
	{
		if((m_particleList[i].active == true) && (m_particleList[i].positionY > /*(float)playerY */+ 1.25f))
		{
			m_particleList[i].active = false;
			m_currentParticleCount--;
...
void ParticleSystemClass::TurnOnParticle(int index)
{
	// 파티클의 속도와 속도 변화를 설정합니다.
	switch (index)
	{
	...
	case 2:
		m_particleVelocity = 1.0f;
		m_particleVelocityVariation = 0.4f;
		break;
	default:
		m_particleVelocity = 1.0f;
		m_particleVelocityVariation = 0.2f;
...
void ParticleSystemClass::TurnOffParticle(int index)
{
	m_particleVelocity = 0.0f;
	m_particleVelocityVariation = 0.0f;
	m_particlesPerSecond = 0.0f;
	m_particleDeviationX = 0.0f;
	m_particleDeviationY = 0.0f;
	m_particleDeviationZ = 0.0f;

	for (int i = 0; i < m_maxParticles; i++)
	{
		m_particleList[i].red = 0.0f;
		m_particleList[i].green = 0.0f;
		m_particleList[i].blue = 0.0f;
	}
}