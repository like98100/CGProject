...
bool LightShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, 
	...
	XMFLOAT3 cameraPosition, XMFLOAT4 specularColor, float specularPower, XMMATRIX rotation)
{
	bool result;
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, 
		lightDirection, ambientColor, diffuseColor, cameraPosition, specularColor, specularPower, rotation);
	if(!result)
	{
		return false;
	}
...
bool LightShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, 
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, 
	ID3D11ShaderResourceView* texture, 
	XMFLOAT3 lightDirection, XMFLOAT4 ambientColor, XMFLOAT4 diffuseColor, XMFLOAT3 cameraPosition, XMFLOAT4 specularColor, float specularPower, XMMATRIX rotation)
{
	HRESULT result;
	...
	XMMATRIX m_rotation = rotation;
	XMVECTOR m_lightDirection_Vector = XMLoadFloat3(&lightDirection);
	XMFLOAT3 m_lightDirection;

	m_rotation = XMMatrixScaling(1.0f, 1.0f, 1.0f); // ³ªÁß¿¡ ºû ¹æÇâ º´½¼µÇ¸é ÀÌ°Å ¼öÁ¤ÇÏ¼À.
	m_lightDirection_Vector = XMVector3TransformCoord(m_lightDirection_Vector, m_rotation);
	m_lightDirection_Vector = XMVector3Normalize(m_lightDirection_Vector);

	XMStoreFloat3(&m_lightDirection, m_lightDirection_Vector);
