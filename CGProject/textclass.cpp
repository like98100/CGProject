	...
	int screenWidth, int screenHeight, XMMATRIX baseViewMatrix)
{
	...
	// Initialize the polygon sentence.
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_polygon, " ", 100, 200, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}

	// Initialize the object sentence.
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_object, " ", 100, 200, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_time, " ", 100, 200, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}

	// Initialize the score sentence.
	result = InitializeSentence(&m_score, 16, device);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_score, " ", 100, 200, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}

	// Initialize the hp sentence.
	result = InitializeSentence(&m_hp, 16, device);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_hp, " ", 100, 200, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}

	// Initialize the screen sentence.
	result = InitializeSentence(&m_screenWidthText, 16, device);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_screenWidthText, " ", 100, 200, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}

	result = InitializeSentence(&m_screenHeightText, 16, device);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_screenHeightText, " ", 100, 200, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}

	result = InitializeSentence(&m_playerPositionX, 16, device);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_playerPositionX, " ", 100, 200, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}

	result = InitializeSentence(&m_playerPositionY, 16, device);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_playerPositionY, " ", 100, 200, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}

	result = InitializeSentence(&m_playerPositionZ, 16, device);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_playerPositionZ, " ", 100, 200, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
void TextClass::Shutdown()
{
	...
	ReleaseSentence(&m_score);

	ReleaseSentence(&m_hp);

	ReleaseSentence(&m_screenWidthText);

	ReleaseSentence(&m_screenHeightText);

	ReleaseSentence(&m_playerPositionX);

	ReleaseSentence(&m_playerPositionY);

	ReleaseSentence(&m_playerPositionZ);
...
bool TextClass::Render(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX orthoMatrix)
{
	...
	result = RenderSentence(deviceContext, m_score, worldMatrix, orthoMatrix);
	if (!result)
	{
		return false;
	}

	result = RenderSentence(deviceContext, m_hp, worldMatrix, orthoMatrix);
	if (!result)
	{
		return false;
	}

	result = RenderSentence(deviceContext, m_screenWidthText, worldMatrix, orthoMatrix);
	if (!result)
	{
		return false;
	}

	result = RenderSentence(deviceContext, m_screenHeightText, worldMatrix, orthoMatrix);
	if (!result)
	{
		return false;
	}

	result = RenderSentence(deviceContext, m_playerPositionX, worldMatrix, orthoMatrix);
	if (!result)
	{
		return false;
	}

	result = RenderSentence(deviceContext, m_playerPositionY, worldMatrix, orthoMatrix);
	if (!result)
	{
		return false;
	}

	result = RenderSentence(deviceContext, m_playerPositionZ, worldMatrix, orthoMatrix);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetPolygon(float polygon, ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_polygon, polygonString, 20, 80, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetTime(float time, ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_time, timeString, 20, 100, r, g, b, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetScore(int score, ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_score, scoreString, 20, 120, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetHP(int hp, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char hpString[16];
	bool result;
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_hp, hpString, 20, 140, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetScreenWidth(ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_screenWidthText, screenString, 20, 160, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetScreenHeight(ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_screenHeightText, screenString, 20, 180, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetPlayerPosX(int x, ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_playerPositionX, posString, 20, 200, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetPlayerPosY(int y, ID3D11DeviceContext* deviceContext)
{
	char tempString[16] = { 0 };
	char posString[16];
	bool result;
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_playerPositionY, posString, 60, 200, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetPlayerPosZ(int z, ID3D11DeviceContext* deviceContext)
{
	char tempString[16] = {0};
	char posString[16];
	bool result;
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_playerPositionZ, posString, 100, 200, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
